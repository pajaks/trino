        ...
        implements HiveMetastore
{
        ...
        try {
            ImmutableList<software.amazon.awssdk.services.glue.model.Table> glueTables = stats.getGetTables()
                    .call(() -> glueClient.getTablesPaginator(builder -> builder
                            ...
                            .flatMap(List::stream))
                    .filter(tableVisibilityFilter).collect(toImmutableList());

            glueTables.stream()
                    .flatMap(glueTable -> convertFromGlueIgnoringErrors(glueTable, databaseName))
                    .forEach(cacheTable);
    ...
    private static Stream<Table> convertFromGlueIgnoringErrors(software.amazon.awssdk.services.glue.model.Table glueTable, String databaseName)
    {
        try {
            return Stream.of(GlueConverter.fromGlueTable(glueTable, databaseName));
        }
        catch (RuntimeException e) {
            handleListingError(e, glueTable.name());
            return Stream.empty();
        }
    }

    private static void handleListingError(RuntimeException e, String tableName)
    {
